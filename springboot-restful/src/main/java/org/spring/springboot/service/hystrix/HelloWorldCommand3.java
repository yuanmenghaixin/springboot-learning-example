...
public class HelloWorldCommand3 extends HystrixCommand<String> {
    ...
    public static void main(String[] args) throws Exception {
        ...
        HelloWorldCommand3 helloWorldCommand = new HelloWorldCommand3("Synchronous-hystrix");
        //同步调用使用execute()代码,效果等同于:helloWorldCommand.queue().get();
        String result = helloWorldCommand.execute();
        System.out.println("result=" + result);

        helloWorldCommand = new HelloWorldCommand3("Asynchronous-hystrix");
        //异步调用,可自由控制获取结果时机,
        ...
        //注册观察者事件拦截
        Observable<String> fs = new HelloWorldCommand3("World").observe();
        //注册结果回调事件
        fs.subscribe(new Action1<String>() {
            ...
            public void call(String result) {
                ...
                //用户对结果做二次处理.
                System.out.println("再次输出结果："+result);
            }
        });
        //注册完整执行生命周期事件
